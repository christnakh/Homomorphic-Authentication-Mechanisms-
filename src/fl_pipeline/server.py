...
class FLServer:
    def __init__(self, model_dim: int = 1000, auth_scheme: str = "BLS",
                 use_encryption: bool = False, aggregation_method: str = "average"):
    ...
    def receive_updates(self, client_updates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.use_encryption:
            if self.he_handler:
                aggregated, _ = self.he_handler.aggregate_encrypted(updates)
                return {"aggregated_update": aggregated, "method": "homomorphic_encrypted"}
        
        if isinstance(updates[0], np.ndarray):
            aggregated = self.aggregator.aggregate(updates)
            return {"aggregated_update": aggregated, "method": self.aggregation_method}
        else:
            return {"aggregated_update": updates[0] if updates else None, "method": "encrypted"}
